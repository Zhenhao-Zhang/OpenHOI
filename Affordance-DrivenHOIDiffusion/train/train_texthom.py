    ...
    with tqdm.tqdm(range(nepoch)) as pbar:
        for epoch in pbar:
            ...
            for item in dataloader:
                ...
                cond_drop_prob = 0.1
                mask = torch.rand(bs) < cond_drop_prob
                # text[mask] = ""
                text = np.array(text, dtype=object)
                text[mask.cpu().numpy()] = ""  # 应用掩码
                text = text.tolist()  # 转换回列表格式（如果需要）
                ...
                obj_feat[mask] = torch.zeros_like(obj_feat[mask])
