...
@hydra.main(version_base=None, config_path="../configs", config_name="contact_config")
    ...
    with tqdm.tqdm(range(nepoch)) as pbar:
        for epoch in pbar:
            ...
            for item in dataloader:
                ...
                tensors_list = []
                for i in text:
                    tensors_list.append(afford_dict[i][:, :1024, :])
                affordance_map = torch.cat(tensors_list, dim=0)
                #print("AffordMap",affordance_map.shape)
                # affordance_map = cov_map.clone()
                ...
                condition = proc_cond_contact_estimator_cov_map(
                    obj_scale, obj_feat, enc_text, 
                    npts, config.contact.use_scale,
                    affordance_map 
                )
