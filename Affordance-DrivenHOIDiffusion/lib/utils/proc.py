...
def proc_cond_contact_estimator(obj_scale, obj_feat, enc_text, npts, use_scale):
    if use_scale:
        ...
        # print(enc_text_expand.shape,obj_scale_expand2.shape)
...
#带有afford-guided的合成
def proc_cond_contact_estimator_cov_map(obj_scale, obj_feat, enc_text, npts, use_scale,affordance_map):
    if use_scale:
        enc_text_expand = enc_text.unsqueeze(1)
        enc_text_expand = enc_text_expand.expand(-1, npts, -1)
        obj_scale_expand2 = obj_scale.unsqueeze(1).unsqueeze(2)
        obj_scale_expand2 = obj_scale_expand2.expand(-1, npts, -1)
        #print(enc_text_expand.shape,obj_scale_expand2.shape,obj_feat.shape,affordance_map.shape)
        #input("wait")
        condition = torch.cat([obj_scale_expand2, obj_feat, enc_text_expand,affordance_map], dim=2)
    else:
        #print(obj_feat.shape, enc_text_expand.shape,affordance_map.shape)
        #input("wait")
        condition = torch.cat([obj_feat, enc_text_expand,affordance_map], dim=2)
    #print(condition.shape)
    return condition
...
def proc_obj_feat_cov(
    ...
    obj_feat_global = obj_feat[:, 0, :1024]
    if use_contact_feat:
        condition = proc_cond_contact_estimator_cov_map(obj_scale, obj_feat, enc_text, npts, use_scale,affordance_map)
        est_contact_map = contact_estimator.decode(condition)
        est_contact_map_plot = est_contact_map.squeeze(-1).clone()
        est_contact_map = (est_contact_map.squeeze(-1) > 0.5).long()
        obj_feat_cov = torch.cat([obj_feat_global, est_contact_map], dim=1)
...
def proc_obj_feat_final_old(
    ...
    obj_feat_cov, est_contact_map, est_contact_map_plot = proc_obj_feat_cov_old(
        contact_estimator, 
        obj_scale, obj_feat, enc_text, npts, 
        use_scale, use_contact_feat, 
    )
