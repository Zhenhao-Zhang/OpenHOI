...
class Diffusion(nn.Module):
    ...
    def ddpm_loop_loss_guidance_contact_and_penetration(
        ...
        for t_idx in pbar:
            ...
            # sample_lhand = sample_lhand.requires_grad_(True)
            # sample_rhand = sample_rhand.requires_grad_(True)
    ...
    #         sample_lhand = sample_lhand.detach().clone().requires_grad_(True)
    #         sample_rhand = sample_rhand.detach().clone().requires_grad_(True)
    #         sample_obj = sample_obj.detach().clone()
    ...
    #             #     loss_type=contact_loss_type
    #             # )
    #             loss = penet_loss #+ 0.1 * contact_loss

    #         pbar.set_postfix({'loss': loss.item()}, refresh=False)
    #         # Ensure tensors require grad
            
    #         # Or recreate them with requires_grad=True if they're model outputs
    #         grad_lhand, grad_rhand = torch.autograd.grad(
    #             outputs=loss,
    #             inputs=[sample_lhand, sample_rhand],
    #             create_graph=False,
    #             retain_graph=False  
    #             # retain_graph=True  # 可以设为False因为一次性计算完了
    #         )
    #         # grad_lhand.requires_grad = True
    #         # grad_rhand.requires_grad = True

    #         grad_lhand_norm = torch.norm(grad_lhand.detach())
    #         grad_rhand_norm = torch.norm(grad_rhand.detach())
    ...
    #         if return_middle and t_idx == 500:
    #             return sample_lhand.detach(), sample_rhand.detach(), sample_obj.detach()
    #     return sample_lhand.detach(), sample_rhand.detach(), sample_obj.detach()
