...
def create_hoi(text,config,lhand_layer,rhand_layer,refiner,texthom,diffusion,clip_model,mpnet,seq_cvae,pointnet,contact_estimator,object_model,affordance_map):
    ...
    for batch_idx in range(batch_num):
        ...
        enc_none_text_batch = enc_none_text[batch_idx*64:(batch_idx+1)*64]
        ...
        coarse_x_lhand, coarse_x_rhand, coarse_x_obj \
            = diffusion.sampling(
                texthom, obj_feat_final, 
                enc_text_batch, enc_none_text_batch, max_nframes, 
                hand_nfeats, obj_nfeats, 
                valid_mask_lhand, 
                valid_mask_rhand, 
                valid_mask_obj, 
        ...
        # coarse_x_lhand, coarse_x_rhand, coarse_x_obj \
        #     = diffusion.sampling_ddpm_loop_loss_guidance_contact_and_penetration(
        #         texthom, obj_feat_final, 
        #         enc_text_batch, enc_none_text_batch, max_nframes, 
        #         hand_nfeats, obj_nfeats, 
        #         valid_mask_lhand, 
        ...
        #         device=torch.device("cuda"),
        #         guidance_rate=2.5,
        #         lhand_layer=lhand_layer,rhand_layer=rhand_layer,obj_verts_org=obj_pc_org_batch,obj_pc_top_idx=obj_pc_top_idx_batch, obj_pc_normal_org=obj_pc_normal_org_batch,cov_map=affordance_map
        #     )
        # sampling_ddpm_loop_loss_guidance_contact_and_penetration
...
def create_hoi_old(text,config,lhand_layer,rhand_layer,refiner,texthom,diffusion,clip_model,mpnet,seq_cvae,pointnet,contact_estimator,object_model,affordance_map):
    #print(OmegaConf.to_yaml(config))
    config = OmegaConf.to_object(config)
    config = edict(config)
    data_config = config.dataset
...
def create_hoi_loss_guidance(text,config,lhand_layer,rhand_layer,refiner,texthom,diffusion,clip_model,mpnet,seq_cvae,pointnet,contact_estimator,object_model,affordance_map, ppx):
    #print(OmegaConf.to_yaml(config))
    config = OmegaConf.to_object(config)
    config = edict(config)
    data_config = config.dataset
    ...
    for batch_idx in range(batch_num):
        ...
        # duration *= 150
        # duration = duration.long()
        # duration = torch.tensor([150],dtype=torch.long)
        # valid_mask_lhand, valid_mask_rhand, valid_mask_obj \
        #     = get_valid_mask_bunch(
        #         is_lhand_batch, is_rhand_batch, 
        #         max_nframes, duration
        #     )
        valid_mask_lhand, valid_mask_rhand, valid_mask_obj = torch.ones((1, 150), dtype=torch.bool).cuda(), torch.ones((1, 150), dtype=torch.bool).cuda(), torch.ones((1, 150), dtype=torch.bool).cuda()
        max_nframes = 150
        #print(affordance_map.shape)
        obj_feat_final = torch.zeros([1,2052]).cuda()
        coarse_x_lhand, coarse_x_rhand, coarse_x_obj \
            = diffusion.sampling_loss_guidance(
                texthom, obj_feat_final, 
                ecn_text_batch, max_nframes, 
                hand_nfeats, obj_nfeats, 
                valid_mask_lhand, 
                valid_mask_rhand, 
                valid_mask_obj, 
        ...
        #     )
    # return refined_x_lhand[0],refined_x_rhand[0],refined_x_obj[0]
    return coarse_x_lhand[0],coarse_x_rhand[0],coarse_x_obj[0]

def create_hoi_projection(text,config,lhand_layer,rhand_layer,refiner,texthom,diffusion,clip_model,mpnet,seq_cvae,pointnet,contact_estimator,object_model,affordance_map, ppx):
    #print(OmegaConf.to_yaml(config))
    config = OmegaConf.to_object(config)
    config = edict(config)
    data_config = config.dataset
    ...
    for batch_idx in range(batch_num):
        ...
        #     )
        # valid_mask_lhand, valid_mask_rhand, valid_mask_obj = torch.ones((1, 150), dtype=torch.bool).cuda(), torch.ones((1, 150), dtype=torch.bool).cuda(), torch.ones((1, 150), dtype=torch.bool).cuda()
        valid_mask_lhand, valid_mask_rhand, valid_mask_obj = torch.ones((1, 10), dtype=torch.bool).cuda(), torch.ones((1, 10), dtype=torch.bool).cuda(), torch.ones((1, 10), dtype=torch.bool).cuda()
        max_nframes = 10
        #print(affordance_map.shape)
        obj_feat_final = torch.zeros([1,2052]).cuda()
        coarse_x_lhand, coarse_x_rhand, coarse_x_obj \
            = diffusion.sampling_projection(
                texthom, obj_feat_final, 
                ecn_text_batch, max_nframes, 
                hand_nfeats, obj_nfeats, 
                valid_mask_lhand, 
